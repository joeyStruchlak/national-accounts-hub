...
serve(async (req) => {
  ...
  try {
    ...
    // === UPDATED: Supabase Storage instead of Google Drive ===
    let storageFileUrl = ''
    let driveFileId = ''
    try {
      const reportContent = `BAS/GST Review Report
Generated: ${new Date().toLocaleDateString('en-AU')}
National Accounts Internal Portal

SUMMARY
Transactions Reviewed: ${totalReviewed}
Issues Found: ${issues.length}
Warnings: ${warnings.length}

FLAGGED ITEMS
${warnings.map((w: any) => `- ${w.contact}: ${w.issue}\n  Recommendation: ${w.recommendation}`).join('\n')}
${issues.map((i: any) => `- CRITICAL: ${i.contact}: ${i.issue}\n  Recommendation: ${i.recommendation}`).join('\n')}
...
Stored securely via Xero OAuth 2.0 | Australian data sovereignty maintained`
      ...
      const filename = `bas-review/${new Date().toISOString().split('T')[0]}_BAS_Review.txt`
      ...
      if (storageError) {
        ...
        storageFileUrl = storageData?.path || ''
        console.log('Saved to Supabase Storage:', storageFileUrl)
      }
    ...
    const { data: run } = await supabaseClient
      ...
      .insert({
        ...
        report_data: { issues, warnings, totalReviewed, totalIssues, aiAnalysis, driveFileId }
      })
      .select()
      .single()
