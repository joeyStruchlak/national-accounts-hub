...
${overdueClients.slice(0, 3).map((c: any) => `- ${c.client}: $${c.amount} (${c.daysOverdue} days overdue)`).join('\n')}
    ...
    let storageFileUrl = ''
    try {
      const reportContent = `Weekly Productivity Report
Generated: ${new Date().toLocaleDateString('en-AU')}
National Accounts Internal Portal

SUMMARY
Invoices Reviewed: ${invoices.length}
...
OVERDUE CLIENTS
${overdueClients.map((c: any) => `- ${c.client}: $${c.amount} (${c.daysOverdue} days overdue)`).join('\n') || 'None'}
...
Stored securely via Xero OAuth 2.0 | Australian data sovereignty maintained`
      ...
      const filename = `productivity-report/${new Date().toISOString().split('T')[0]}_Productivity_Report.txt`
