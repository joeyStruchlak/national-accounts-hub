...
export function ReportModal({ open, onOpenChange, type, report, realData }: ReportModalProps) {
  if (!type) return null;

  // Use real data if available, otherwise fall back to mock
 const isRealData = realData && type === 'bas'
const aiAnalysis = realData?.aiAnalysis || null
  ...
  // Build summary from real data or mock
  const summary = isRealData ? [
    { label: "Transactions Reviewed", value: String(realData.totalReviewed) },
    { label: "Issues Detected", value: String(realData.issues.length) },
    { label: "Warnings", value: String(realData.warnings.length) },
    { label: "Compliance Rate", value: realData.totalIssues === 0 ? "100%" : `${Math.round((1 - realData.totalIssues / realData.totalReviewed) * 100)}%` },
  ] : [
  ...
  // Build flagged items from real data or mock
  ...
  // Build actions from real data or mock
const actions: string[] = isRealData ? [
    ...realData.issues.map(i => i.recommendation),
    ...(realData.warnings.length > 0 ? ["Add meaningful descriptions to all flagged transactions for ATO audit trail compliance."] : []),
    realData.totalIssues === 0 ? "All transactions passed GST review. No action required." : "Partner sign-off required before BAS lodgement.",
  ] : [
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl max-h-[85vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            {isRealData ? "Live data from Xero · All data verified" : "Generated just now · All data verified"}
          </DialogDescription>
        </DialogHeader>
        ...
        {flagged.length === 0 && (
          <div className="mt-4 rounded border border-success/30 bg-success/5 px-4 py-3">
            <p className="text-sm text-success font-semibold">✓ All clear — no issues detected</p>
          </div>
        )}
        ...
        <div className="mt-4">
          ...
          <div className="mt-4 rounded border border-accent/30 bg-accent/5 px-4 py-3">
            <h4 className="text-sm font-semibold text-primary mb-2">🤖 AI Analysis</h4>
            <p className="text-sm text-foreground leading-relaxed whitespace-pre-line" dangerouslySetInnerHTML={{ __html: aiAnalysis.replace(/\*\*(.*?)\*\*/g, '<strong>$1</strong>') }} />
          </div>
        )}
