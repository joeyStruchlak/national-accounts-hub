...
const phases: Record<AutomationType, string[]> = {
  ...
  productivity: ["Connecting to practice manager…", "Pulling invoice data…", "Calculating collections…", "Aggregating revenue figures…", "Generating report…"],
};
...
export default function AIAutomation() {
  ...
  const runAutomation = useCallback(async (type: AutomationType) => {
    ...
    setRunStates((prev) => ({
      ...prev,
      [type]: { running: true, phase: steps[0], done: false, report: null, activeStep: 0 },
    }));
      ...
      if (type === "bas") {
        ...
        if (data.success) {
          ...
          const reportText = total > 0
            ? `Review complete. ${data.totalReviewed} transactions reviewed. ${issueCount} critical issues and ${warningCount} warnings found. ${data.warnings?.map((w: { contact: string; issue: string }) => `${w.contact}: ${w.issue}`).join(". ")}`
            : `Review complete. ${data.totalReviewed} transactions reviewed. No issues found. All clear.`;

          setRunStates((prev) => ({
            ...prev,
            [type]: { running: false, phase: "", done: true, report: reportText, activeStep: 4 },
          }));
        ...
        const response = await fetch(
          `${import.meta.env.VITE_SUPABASE_URL}/functions/v1/productivity-report`,
          {
            method: "POST",
            headers: {
              Authorization: `Bearer ${import.meta.env.VITE_SUPABASE_ANON_KEY}`,
        ...
        if (data.success) {
          const reportText = `Report complete. ${data.recordsReviewed} invoices reviewed. Total invoiced: $${data.totalInvoiced.toFixed(0)}. Collected: $${data.totalPaid.toFixed(0)}. Outstanding: $${data.totalOutstanding.toFixed(0)}. ${data.overdueCount > 0 ? `${data.overdueCount} overdue invoices need attention.` : "No overdue invoices."}`
          ...
          setRealData({
            ...
            issues: data.flags || [],
            warnings: data.overdueClients?.map((c: { client: string; amount: number; daysOverdue: number }) => ({
              contact: c.client,
        ...
        setRunStates((prev) => ({
          ...prev,
          [type]: { running: false, phase: "", done: true, report: reportOutputs[type], activeStep: 4 },
        }));
      }
    ...
    <AppLayout>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
        {automations.map((a) => {
          ...
          return (
            <Card key={a.key} className="shadow-premium-lg border-border border-t-2 border-t-warning relative overflow-hidden flex flex-col">
              <CardContent className="p-6 flex flex-col flex-1">
                ...
                >
                  {state.running ? (
                    ...
                    <><Play className="mr-2 h-4 w-4" />Run {a.key === "bas" ? "Review" : a.key === "payroll" ? "Check" : "Report"}</>
                  )}
                </Button>
                ...
                <div className="flex items-center gap-2 mb-3">
                  <span className={cn("h-2 w-2 rounded-full shrink-0", a.connected ? "bg-accent animate-pulse" : "bg-muted-foreground")} />
                  <span className="text-[10px] text-muted-foreground">
                    <span className={a.connected ? "text-accent" : ""}>{a.connected ? "Connected to Xero OAuth 2.0" : "Disconnected"}</span>
                  </span>
                </div>

                <div className="mt-auto pt-3 border-t border-border flex items-center justify-between">
                  <p className="text-[11px] text-muted-foreground">Last run: {a.lastRun}</p>
                  <span className={cn("inline-flex items-center gap-1 text-[11px] font-semibold", a.lastResultType === "success" ? "text-accent" : "text-warning")}>
                    <span className={cn("h-1.5 w-1.5 rounded-full", a.lastResultType === "success" ? "bg-accent" : "bg-warning")} />
                    {a.lastResult}
                  </span>
                </div>
              </CardContent>
      ...
      <ReportModal
        ...
        realData={reportModal.type === "bas" || reportModal.type === "productivity" ? realData : null}
      />
                ...
                <>
                  ...
                  >
                    ...
                    <TableCell className="text-sm text-right">
                      <span className={cn("font-semibold", entry.issues > 0 ? "text-warning" : "text-accent")}>{entry.issues}</span>
                    </TableCell>
                    <TableCell><StatusBadge status={entry.status} /></TableCell>
                    <TableCell>
                      {expandedRow === entry.id ? <ChevronUp className="h-4 w-4 text-muted-foreground" /> : <ChevronDown className="h-4 w-4 text-muted-foreground" />}
                    </TableCell>
                  </TableRow>
